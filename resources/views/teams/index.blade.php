                ...
                <div class="p-6 text-gray-900">
                    @if($teams->isEmpty())
                        <div class="py-4 text-sm text-gray-600">
                            No teams yet.
                            @if($units->isEmpty())
                                <a href="{{ auth()->user()->hasChosenStarter() ? route('scan.index') : route('starter.show') }}" class="text-indigo-600 hover:text-indigo-500 font-medium">Get a unit first</a>.
                            @else
                                <a href="{{ route('teams.create') }}" class="text-indigo-600 hover:text-indigo-500 font-medium">Create your first team</a>.
                            @endif
                        </div>
                    @else
                        @foreach ($teams as $team)
                            <div class="border-b border-gray-100 py-4 last:border-b-0">
                                ...
                                <div class="mt-3 grid grid-cols-1 md:grid-cols-5 gap-3">
                                    @forelse ($team->teamUnits->sortBy('position') as $slot)
                                    ...
                                    @empty
                                        <p class="text-sm text-gray-600">No units in this team yet.</p>
                                    @endforelse
                                </div>
                            </div>
                        @endforeach
